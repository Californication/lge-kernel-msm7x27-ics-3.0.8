...
//	arg.input = NULL;
	...
	oem_rapi_client_streaming_function(rpc_client, &arg, &ret);

	if(ret.output == NULL || ret.out_len == NULL)
	{
		printk(KERN_ERR "%s, output is NULL, ret.output: %p, ret.out_len: %p\n", __func__, ret.output, ret.out_len);
		return -1;
	}
