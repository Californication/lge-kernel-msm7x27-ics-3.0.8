...
static void monitor_ghost_finger(struct work_struct *work){
	...
	if(!is_touch_suspend){
		if(ghost_touch_cnt >= 45){
			printk(KERN_ERR "<MELFAS> ghost finger pattern DETECTED! : %d \n", ghost_touch_cnt);
		...
		schedule_delayed_work(&ts->ghost_monitor_work, msecs_to_jiffies(HZ * 50));
	}
	ghost_touch_cnt = 0;
	ghost_x = 1000;
	ghost_y = 1000;
		...
		if (ret < 0) {
			...
			if (touchType == TOUCH_SCREEN) {
				...
				if (touchState)
					...
					{
						if(ghost_x + 40 >= posX && ghost_x - 40 <= posX){
							if(ghost_y + 40 >= posY && ghost_y - 40 <= posY){
								ghost_touch_cnt++;
							}
						}			
					}
...
static int melfas_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (is_downloading == 0) {
		...
		#endif
		ret = flush_delayed_work_sync(&ts->ghost_monitor_work);
