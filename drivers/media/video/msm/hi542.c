...
static int32_t hi542_sensor_setting(int update_type, int rt)
{
	...
	CDBG("###### %s: %d Enter - update_type[%d] rt[%d]\n",__func__, __LINE__, update_type, rt);
...
/*LGE_CHANGE_S : Hi542 sensor stabilization
  apply X-shutdown mode,sensor reset stabilization*/
	...
	hi542_i2c_write_b_sensor(0x03d4, 0x28);//20120416 ryu 18);/* LDO level */20120418 ryu 28
	msleep(3);
/* LGE_CHANGE_E : Hi542 sensor stabilization */	
...
/* LGE_CHANGE_E : Hi542 sensor stabilization */	
		...
		if (rt == RES_PREVIEW)
		{
			if (!CSI_CONFIG) 
			{
				...
				hi542_csi_params.settle_cnt = 0x21; //1c//14; settle count 20120418 ryu add
...
static int32_t hi542_power_down(void)
{
	...
	if (hi542_ctrl->curr_step_pos != 0) {
		if(hi542_ctrl->curr_step_pos > hi542_nl_region_boundary1) {
			rc = hi542_move_focus(MOVE_FAR, (hi542_ctrl->curr_step_pos - hi542_nl_region_boundary1));
			mdelay(10);
			rc = hi542_move_focus(MOVE_FAR, hi542_nl_region_boundary1);
		}
		else {
			rc = hi542_move_focus(MOVE_FAR,	hi542_ctrl->curr_step_pos);
		}
...
//	gpio_free(hi542_ctrl->sensordata->sensor_reset);
	printk("hi542_release completed\n");
	if (hi542_ctrl->sensordata->vcm_enable) {
